...
export default function AddBlockButton({ onSelect, placeholder }: Props) {
  ...
  const handleDrop = (e:any) => {
    onSelect(JSON.parse(e))
  };

  const renderButton = () => {
    if (placeholder) {
      return <PlaceholderButton onClick={handleButtonClick} onDrop={handleDrop} />;
    } else {
      return <DividerButton buttonElement={buttonElement} onDrop={handleDrop} onClick={handleButtonClick} />;
    }
  };
